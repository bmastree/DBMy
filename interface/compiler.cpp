...
template <class T> struct Record_avl {
    ...
    char data[400]{};
    Record_avl() = default;
    Record_avl(T key_, string &data_) {
        string temp_key = convertToString(key_);
        temp_key = temp_key.size()<100?temp_key:temp_key.substr(0, 98);
        string temp_data = data_.size()<300?data_:data_.substr(0, 298);
        strcpy(this->key, temp_key.c_str());
        strcpy(this->data, temp_data.c_str());
    }
    string getData() {
...
template<class T>
Record_avl<T> getRecord_avl(string line) {
    string id = "", data="", field;
    istringstream lineStream(line);
    getline(lineStream, id, ',');
    data = line.substr(id.length() + 1);
    return Record_avl<T>(convert<T>(id), data.c_str());
}

template<class T>
Record_avl<T> getRecord_avl(string line, int idPos) {
    string id = "", data="", field;
    istringstream lineStream(line);
    int count = 0;
...
bool writeTables(string tablename, int posKey, string typeKey) {
    string filename = "tablas.txt";
    ...
    if (!tableExists) {
        ...
        outfile << tablename << "," << posKey << "," << typeKey << "\n";
        outfile.close();
        cout << "Tabla '" << tablename << "' añadida al archivo." << endl;
    } else {
        cout << "La tabla '" << tablename << "' ya existe en el archivo." << endl;
    }
    return !tableExists;
}

pair<string, int> readTables(string tablename) {
    ...
    while (getline(infile, line)) {
        stringstream ss(line);
        string existingTableName, typeKey;
        int posKey;
        getline(ss, existingTableName, ',');
        ss >> posKey;
        ss.ignore();
        ...
        if (existingTableName == tablename) {
            infile.close();
            return make_pair(typeKey, posKey);
        }
    }
...
private:
    ...
    vector<string> validateCreateTable(const std::string& statement) {
        ...
        static const std::regex createTableRegexAVL(R"(create\s+table\s+(\w+)\s+from\s+file\s+"([^"]+)\"\s+using\s+index\s+(\w+)\("(\w+)\"\))");
        std::smatch matches;
        vector<string> res;
